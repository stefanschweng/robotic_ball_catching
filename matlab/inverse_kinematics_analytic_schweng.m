function [success,q,all_solutions] = inverse_kinematics_analytic_schweng(x_d,param,find_minimum_norm,current_q)
  elbow_up=true;
  if(nargin<3)
  ...
  tmp_solution_size=17;
  solution=zeros(7,tmp_solution_size);
  ...
  if(find_minimum_norm)
    ...
    best = zeros(7,1);
    best_vecnorm_init_value = 9999;
    best_vecnorm=best_vecnorm_init_value;
    ...
    for i=-N:N
      for j=-N:N
        for k=-N:N
          ...
          % find new best and check validity of solutions
          tmp(:)=wrap(tmp(:));
          for f=1:tmp_solution_size
              candidate_vecnorm=vecnorm(tmp(:,f)-current_q);
              candidate=tmp(:,f);
              
              if ((candidate_vecnorm < best_vecnorm) &&...
                      all(candidate<=param.q_limit_upper) &&...
                      all(candidate>=param.q_limit_lower))
                  best=candidate;
                  best_vecnorm=candidate_vecnorm;
              end
          end
    ...
    if best_vecnorm~=best_vecnorm_init_value
       success=true; 
    end
